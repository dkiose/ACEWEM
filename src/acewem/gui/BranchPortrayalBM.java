...
public class BranchPortrayalBM extends SimpleEdgePortrayal2D {

	private final ACEWEMmodel market;

	public BranchPortrayalBM(final ACEWEMmodel market) {
		this.market = market;
	...
	public void draw(final Object object, final Graphics2D graphics,
		...
		for (int h = 0; h < Settings.HOURS; h++) {
			voltageDiff = voltageDiff
					+ Math.abs(market.getAngleList(Settings.BM)[nodeTo.getIdNum() - 1][h]
							- market.getAngleList(Settings.BM)[nodeFrom.getIdNum() - 1][h]);
		}
		...
		//graphics.setStroke(new BasicStroke(1));
		graphics.setColor(new Color((float) 1.0, (float) 0.0,
				1 - congestionCoeff));
