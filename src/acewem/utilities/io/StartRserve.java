...
class RegistryHog extends Thread {
	...
	RegistryHog(final InputStream is, final boolean capture) {
		this.is = is;
...
public class StartRserve {
	...
	public static boolean checkLocalRserve() {
		...
		if (osname != null && osname.length() >= 7
			...
			System.err
					.println("Windows: query registry to find where R is installed ...");
			String installPath = null;
			try {
				final Process rp = Runtime.getRuntime().exec(
						"reg query HKLM\\Software\\R-core\\R");
				final RegistryHog regHog = new RegistryHog(rp.getInputStream(),
						true);
				rp.waitFor();
				regHog.join();
				...
				System.err
						.println("ERROR: unable to run REG to find the location of R: "
								+ rge);
				return false;
			}
			if (installPath == null) {
				System.err
						.println("ERROR: canot find path to R. Make sure reg is available and R was installed with registry settings.");
				return false;
			}
			return launchRserve(installPath + "\\bin\\R.exe");
		}
		return (launchRserve("R")
				|| /* try some common unix locations of R */((new File(
	 ...
	 * @param rargs
	 *            arguments are are to be passed to R (e.g. --vanilla -q)
	 * @return <code>true</code> if Rserve is running or was successfully
	 *         started, <code>false</code> otherwise.
	 */
	public static boolean doInR(final String todo, final String Rcmd,
			final String rargs, final StringBuffer out, final StringBuffer err) {
		try {
			Process p;
			...
			// don't ...
			final StreamHog error = new StreamHog(p.getErrorStream(),
					(err != null));
			final StreamHog output = new StreamHog(p.getInputStream(),
					(out != null));
			if (err != null) {
				error.join();
			}
			...
			if (!isWindows) /*
							 * on Windows the process will never return, so we
							 * cannot wait
							 */{
				p.waitFor();
			}
	 ...
	 *            command necessary to start R
	 * @param http_proxy
	 *            http://login:password@proxy:port string to enable internet
	 *            access to rforge server
	 * @return success
	 */
	public static boolean installRserve(final String Rcmd,
			final String http_proxy, String repository) {
		if (repository == null || repository.length() == 0) {
			repository = "http://cran.cict.fr/";
		}
		System.err.print("Install Rserve from " + repository
				+ " ... (http_proxy=" + http_proxy + ") ");
		final boolean ok = doInR(
				(http_proxy != null ? "Sys.setenv(http_proxy='" + http_proxy
						+ "');" : "")
						+ "install.packages('Rserve',repos='"
						+ repository
						+ "')", Rcmd, "--vanilla", null, null);
		if (!ok) {
	 ...
	 */
	public static boolean isRserveInstalled(final String Rcmd) {
		final StringBuffer result = new StringBuffer();
		final boolean done = doInR(
				"i=installed.packages();is.element(set=i,el='Rserve')", Rcmd,
				"--vanilla -q", result, result);
		if (!done) {
			return false;
		}
	...
	public static boolean isRserveRunning() {
		try {
			...
			System.err.println("First connect try failed with: "
					+ e.getMessage());
		}
		return false;
	}
	...
	public static boolean launchRserve(final String cmd) {
		return launchRserve(cmd, /* null, */"--no-save --slave",
				"--no-save --slave", false);
	}
	 ...
	 *         started, <code>false</code> otherwise.
	 */
	public static boolean launchRserve(final String cmd, /* String libloc, */
			final String rargs, final String rsrvargs, final boolean debug) {
		System.err.println("Waiting for Rserve to start ...");

		// manually added !!!!
		// if(!doInR("\"Rser\" %in% rownames(installed.packages())",cmd, rargs,
		// null, null)) {
		// doInR("install.packages(\"Rserve\")",cmd, rargs, null, null);
		// }
		// !!!!!!!!!!!!!!

		final boolean startRserve = doInR("library(" + /*
														 * (libloc != null ?
														 * "lib.loc='" + libloc
														 * + "'," : "") +
														 */"Rserve);Rserve("
		...
		if (startRserve) {
			System.err
					.println("Rserve startup done, let us try to connect ...");
		} else {
		...
		while (attempts > 0) {
			try {
				...
				if (rsrvargs.contains("--RS-port")) {
					final String rsport = rsrvargs.split("--RS-port")[1].trim()
							.split(" ")[0];
